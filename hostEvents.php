...
$titleErr = $artistErr = $descriptionErr =""; //defining variables
$connection = connect(); //defining variables
$userID=$_COOKIE["userID"];

$title = $categoryID = $artist = $description =""; //defining variables

$sql1 = "SELECT Category_id, Category FROM `category`";
$result1 = mysqli_query($connection, $sql1);

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty(test_input($_POST["title"]))){ //test for input in the input box: title; this process is repeated for all variables
        $titleErr = "* Please enter a Title for your event"; //display corresponding error message; this process is repeated for all variables
    }
    else{
        $title = test_input($_POST["title"]); //associate the variable $title with the input; this process is repeated for all variables
        $categoryID = test_input($_POST["categoryID"]);
        if (empty(test_input($_POST["artist"]))){
            ...
            if(empty(test_input($_POST["description"]))){
                ...
                $sql2 = "SELECT Event_id FROM `event` ORDER BY Event_id DESC LIMIT 1"; // find the largest number in the database for event id
                $result2 = mysqli_query($connection, $sql2);
                $row2 = mysqli_fetch_array($result2);
                $eventID=$row2['Event_id']+1; //establishing new event id for data created
                $sql3 = "INSERT INTO `event`(`Title`, `Event_id`, `Category_id`, `Artist`, `Description`)"." VALUES ('$title','$eventID','$categoryID','$artist','$description')"; //creating a new entry in event
                $result3 = mysqli_query($connection,$sql3) or die('Error making saveToDatabase query');
                setcookie("eventID", "$eventID",0,'/'); //setcookie for eventID to be used in addShow.php
                $sql4 = "INSERT INTO `organiser`(`Event_id`, `User_id`)"." VALUES ('$eventID','$userID')"; // creating a new entry in organiser
