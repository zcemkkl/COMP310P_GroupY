...
WHERE sales.User_id = '$userID'"; //sql to search for the event selected.
$result1 = mysqli_query($connection, $sql1);

// there is no error handling in feedback because, the feedback form gives options for the compulsory components and for comments, it can be left blank.
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $eventID = test_input($_POST["eventID"]);
    $rating = test_input($_POST["rating"]);
    $comments = test_input($_POST["comments"]);
    $date = date("Y/m/d");
    $sql2 = "INSERT INTO `review`(`Event_id`, `User_id`, `Rating`, `Description`, `Review_date`)"." VALUES ('$eventID','$userID','$rating','$comments', '$date')"; //create a new entry in the field: review
...
<div class="feedback">
    ...
    <form method="post">
        <?php
        echo "Event: "; //create a Select input for the field: Event
        ...
        <br>
        <?php
        echo "Rating: "; //create a Select input for the field: Rating
        ...
        <br>
        <?php echo "Comments: "?>
        <input type="text" name="comments" value="<?php echo $comments;?>" id="commentsBox"> <!-- Create a text input for field: comments -->
        <br>
